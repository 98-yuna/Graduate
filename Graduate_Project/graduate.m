% https://github.com/98-yuna/Graduate/tree/main/Graduate_Project
...
datastore1 = signalDatastore("C:\Users\qkrtl\AppData\Local\Temp\ModulationFiles",'SignalVariableNames',["label","N"])
...
% 데이터스토어에 저장되어있는 입력신호를 tall배열로 변환
% 데이터양이 너무 많아 tall형 배열로 변환해야함
% 위에서 변환된 신호의 label값 모음
total_Labels=gather(tall(transform(input_signal, @import_label)));


numFrames = length(total_Labels);
...
trainSet = zeros(numTrainFrames,1);  % 훈련셋의 데이터를 저장하기 위해 zeros함수를 사용하여 8000x1 배열 생성
testSet = zeros(numTestFrames,1);    % 테스트셋 데이터를 저장하기 위해 zeros함수를 사용하여 2000x1 배열 생성
trainFrameCnt = 0;
testFrameCnt = 0;

for modType = 1:numModulationTypes  % 변조 신호 종류 반복
  modLabel = find(string(total_Labels) == string(modulationTypes(modType))); %datastore label값 중 변조 신호 종류와 같은거 찾기
  numFrames = length(modLabel);  % 변조 신호 각각 갯수 반환
  
  numModTrainFrames = numFrames*percentTrainingSamples/100; %80만큼 프레임 나누기 =1600
  numModTestFrames = numFrames*percentTestSamples/100; %20만큼 프레임 나누기 =400
  
  trainSet((1:numModTrainFrames)+trainFrameCnt,1) = modLabel(1:numModTrainFrames,1);  % label 배열 넣기
  trainFrameCnt = trainFrameCnt + numModTrainFrames;  %1600개마다 변조신호 종류 달라짐
  
  testSet((1:numModTestFrames)+testFrameCnt,1) = modLabel(1:numModTestFrames,1);
  testFrameCnt = testFrameCnt + numModTestFrames; %400개마다 변조신호 종류 달라짐
end

trainDS = subset(input_signal, trainSet);  %train, test 나눠서 데이터스토어에 저장
testDS = subset(input_signal, testSet);
